...
| Path | Scope |
|------|-------|
| `gateway/` | 🌐 Messaging gateway — platform adapters (Telegram, Discord, Slack, etc.), session lifecycle, delivery routing |
| `plugins/` | 🔌 Plugin system — model providers, memory backends, browser automation, kanban, image/video gen |
| `tools/` | 🛠️ Tool registry and built-in tool implementations, terminal environments |
| `ui-tui/` | 🖥️ Ink (React) terminal UI — `hermes --tui`, TypeScript rendering over JSON-RPC |
| `agent/` | 🤖 Agent core internals — provider adapters, memory, context engine, auxiliary clients, conversation compression |
| `hermes_cli/` | ⌨️ CLI subcommands, config management, skin engine, wizard, plugins loader |
| `cron/` | ⏰ Scheduler — `jobs.py`, `scheduler.py` for recurring agent runs |
| `skills/` | 📚 Built-in skill definitions bundled with the repo |
