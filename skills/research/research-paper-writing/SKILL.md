...
Covers the complete writing pipeline: Introduction → Related Work → Method → Experiments → Conclusion → Abstract → Title, plus professional LaTeX tooling (TikZ, `booktabs`, `algorithm2e`, `siunitx`), template setup, figure standards, and broader impact statements.

Key principles:
- **Write the abstract last**, after the paper body is complete
- **Every claim in the paper must trace to a specific result**
- **Baselines must be strong** — weak baselines are the #1 rejection reason
- **Method diagram** (TikZ) and **pseudocode** (`algorithm2e`) for complex approaches
- **Self-contained captions** — reader should understand figures without main text
- **Compile after every section** — never accumulate LaTeX errors
...
- Dataset documentation is mandatory, not optional (see Datasheets, [Phase 5: Paper Drafting](references/phase5-paper-drafting.md))
...
**Legacy note:** This skill supersedes an earlier `ml-paper-writing` skill (consolidated into this one). It contains the full experiment/analysis pipeline and autoreason methodology.
...
| Missing broader impact statement | See [Phase 5: Paper Drafting](references/phase5-paper-drafting.md). Most venues require it. "No negative impacts" is almost never credible. |
